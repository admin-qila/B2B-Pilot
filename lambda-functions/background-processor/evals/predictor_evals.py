...
import json
import base64
from predictor import product_classification, upload_file_from_base64
...
def call_product_classification(image_path):
    ...
    result = product_classification(
        ...
        user_prompt=(
            "The Reference images shows 2 products. Is the user image showing the same "
            "brand, product line and packaging design of either 1? The user is allowed "
            "to upload box or receipts of either of the product."
        ),
    )
    return result
...
def evaluate_image(image_path, ground_truth):
    ...
    result = call_product_classification(image_path)
    ...
    for key in ["is_product_match", "request_new_image", "multiple_products", "partial_or_occluded", "blurry_or_unclear"]:
...
def evaluate_folder(folder_path, ground_truth_dict, output_file="eval_results_bad_2.json"):
...
if __name__ == "__main__":
    ...
    with open("RR_FSE_test/ground_truth_default/rr_supergreen_bad.json", "r") as f:
        ground_truth = json.load(f)

    # Evaluate images in the specified folder
    evaluate_folder("RR_FSE_test/RR-Bad", ground_truth)